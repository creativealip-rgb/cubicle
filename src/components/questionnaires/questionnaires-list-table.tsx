  ...
  return (
    <>
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/40 hover:bg-muted/40 border-b border-border/80">
              <TableHead>
                <SortableHeader
                  label={t("Kuesioner", "Questionnaire")}
                  dir={dirFor("name")}
                  onClick={() => toggle("name")}
      ...
      <div className="md:hidden space-y-3">
        {sorted.map((q) => (
          <div key={q.id} className="rounded-xl border border-border/80 bg-card p-3.5 space-y-2.5 shadow-xs transition-colors hover:bg-muted/40">
            <div className="flex items-start justify-between gap-3">
              ...
              <Badge variant="outline" className="text-[10px] rounded-md border-border/70 h-5 px-2 font-normal text-muted-foreground shrink-0">
                {q.fieldCount} {t("kolom", "fields")}
              </Badge>
            </div>

            <div className="flex items-center justify-between text-xs text-muted-foreground pt-1 border-t border-border/60">
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="gap-1 text-[10px] font-medium border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 rounded-full px-2 py-0 h-5">
                  <span className="h-1 w-1 rounded-full bg-emerald-600" />
                  {q.submitted} {t("selesai", "done")}
                </Badge>
                {q.pending > 0 && (
                  <Badge variant="outline" className="gap-1 text-[10px] font-medium border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-400 rounded-full px-2 py-0 h-5">
                    <span className="h-1 w-1 rounded-full bg-amber-600" />
                    {q.pending} {t("tertunda", "pending")}
                  </Badge>
                )}
              </div>
