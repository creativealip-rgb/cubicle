  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border p-8 space-y-6">
      <div className="border-b pb-4">
        ...
        {proposal.validUntil && (
          <p className="text-sm text-slate-500 mt-1">{t("Berlaku sampai", "Valid until")} {proposal.validUntil}</p>
        )}
      </div>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-right w-32">{t("Harga satuan", "Unit price")}</TableHead>
            <TableHead className="text-right w-32">{t("Jumlah", "Amount")}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {items.map((li, i) => (
      ...
      {parseFloat(proposal.downPaymentPercent) > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
          ...
          <div className="text-sm">
            <p className="font-medium text-blue-900">{t("Uang muka untuk memulai", "Down payment to get started")}</p>
            <p className="text-blue-700 mt-1">
              {proposal.downPaymentPercent}% ({formatMoney(
                parseFloat(proposal.total) * (parseFloat(proposal.downPaymentPercent) / 100),
                proposal.currency
              )} {t("dibayar saat proposal disetujui untuk memulai pekerjaan.", "is due upon acceptance to begin work.")}
            </p>
          </div>
        </div>
      )}
