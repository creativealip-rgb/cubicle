    ...
    <div className={cn("flex h-14 items-center border-b border-border/80 px-3", collapsed ? "lg:justify-center" : "justify-between")}>
      {!collapsed && (
        <Link href="/app/dashboard" className="flex min-w-0 items-center gap-2 font-semibold text-sidebar-foreground">
          ...
          <img src="/logo-header.png" alt="Cubiqlo" className="h-8 w-auto object-contain" />
        </Link>
      )}
      {collapsed && (
        <Link href="/app/dashboard" className="hidden lg:flex">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/logo-icon.png" alt="Cubiqlo" className="h-8 w-8 rounded-lg object-contain shadow-2xs" />
        </Link>
      )}
    ...
    <div className="space-y-2 border-t border-border/80 p-3">
      ...
      {collapsed ? (
        <button
          ...
          className="mx-auto flex min-h-11 min-w-11 w-full items-center justify-center rounded-md border bg-white text-[10px] font-semibold text-muted-foreground hover:text-foreground disabled:opacity-50"
        >
          {lang === "id" ? "ID" : "EN"}
        </button>
      ) : (
        <div className="flex items-center justify-between gap-2">
          ...
          <div
            className={cn(
              "flex items-center rounded-md border bg-white p-0.5 text-[11px]",
              pending && "opacity-50",
            )}
          >
            {(["id", "en"] as const).map((code) => (
              <button
                ...
                className={cn(
                  "h-7 min-w-7 rounded px-1.5 text-[10px] font-semibold uppercase",
                  lang === code
