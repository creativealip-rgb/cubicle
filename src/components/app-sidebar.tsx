...
export function AppSidebar({ collapsed, onToggle, badgeCounts, workspaceRole }: AppSidebarProps) {
  ...
  return <aside className={cn(
    ...
    <div className={cn("flex h-14 items-center border-b border-sidebar-border px-3", collapsed ? "lg:justify-center" : "justify-between")}>
      ...
      <Button variant="ghost" size="icon" className="h-8 w-8 text-sidebar-foreground hover:bg-sidebar-accent lg:hidden" onClick={() => setMobileOpen(false)} aria-label={t("Tutup menu", "Close menu")}><X className="h-4 w-4" /></Button>
      {!collapsed && <Button variant="ghost" size="icon" className="hidden h-8 w-8 text-sidebar-foreground hover:bg-sidebar-accent lg:flex" onClick={onToggle} aria-label={t("Ciutkan sidebar", "Collapse sidebar")}><PanelLeftClose className="h-4 w-4" /></Button>}
      {collapsed && <Button variant="ghost" size="icon" className="absolute -right-3 top-3 hidden h-8 w-8 rounded-full border bg-background text-sidebar-foreground shadow-sm hover:bg-sidebar-accent lg:flex" onClick={onToggle} aria-label={t("Bentangkan sidebar", "Expand sidebar")}><PanelLeft className="h-3 w-3" /></Button>}
    </div>
    ...
    <div className="space-y-2 border-t border-sidebar-border p-3">
      ...
      {collapsed ? <button type="button" onClick={() => setLang(lang === "id" ? "en" : "id")} disabled={pending} aria-label={t("Ganti ke Bahasa Inggris", "Switch to Indonesian")} className="mx-auto flex h-7 w-full items-center justify-center rounded-md border bg-white text-xs font-semibold text-muted-foreground hover:text-foreground disabled:opacity-50">{lang === "id" ? "ID" : "EN"}</button> : <div className="flex items-center justify-between"><p className="text-xs text-muted-foreground">Cubiqlo v{process.env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0"}</p><div className={cn("flex items-center rounded-md border bg-white p-0.5 text-[11px]", pending && "opacity-50")}>{(["id", "en"] as const).map((code) => <button key={code} type="button" onClick={() => setLang(code)} disabled={pending} aria-label={code === "id" ? "Bahasa Indonesia" : "English"} className={cn("h-5 rounded px-2 font-semibold uppercase", lang === code ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>{code}</button>)}</div></div>}
    </div>
  </aside>;
}
