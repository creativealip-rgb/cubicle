      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="overflow-x-auto -mx-1 px-1">
          <TabsList className="h-auto min-h-9 w-auto inline-flex justify-start gap-1 bg-muted/60 p-1">
            ...
            <TabsTrigger value="projects" className="gap-1.5 px-3 py-1 text-xs font-medium sm:text-sm data-[state=active]:shadow-sm">
              {t("Proyek", "Projects")}
              <span className="rounded-full bg-background px-1.5 py-0.5 text-[10px] tabular-nums text-muted-foreground">
                {projectsCount}
              </span>
            </TabsTrigger>
            <TabsTrigger value="invoices" className="gap-1.5 px-3 py-1 text-xs font-medium sm:text-sm data-[state=active]:shadow-sm">
              {t("Invoice", "Invoices")}
              <span className="rounded-full bg-background px-1.5 py-0.5 text-[10px] tabular-nums text-muted-foreground">
                {invoicesCount}
              </span>
