...
export function ClientOverview({ client, projects, invoices, currency, trackedMinutes, outstanding, invoiced, editAction, projectAction, invoiceAction, t }: {
  ...
  currency: string; trackedMinutes: number; outstanding: number; invoiced: number;
  editAction: React.ReactNode; projectAction?: React.ReactNode; invoiceAction?: React.ReactNode;
  t: (id: string, en: string) => string;
}) {
  const metrics = [
    [t("Proyek Aktif", "Active Projects"), String(projects.filter((p) => p.status === "active").length), FolderKanban],
...
function OverviewList({ title, href, empty, action, children, t }: { title: string; href: string; empty: string; action?: React.ReactNode; children: React.ReactNode; t: (id: string, en: string) => string }) { const has = Array.isArray(children) ? children.length > 0 : Boolean(children); return <Card className="rounded-xl"><CardContent className="flex min-h-64 flex-col p-4"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold">{title}</h2><Button variant="ghost" size="sm" asChild><Link href={href}>{t("Lihat semua", "View all")}</Link></Button></div><div className="flex-1">{has ? children : <div className="flex h-full min-h-36 flex-col items-center justify-center gap-3 text-center"><p className="text-sm text-muted-foreground">{empty}</p>{action}</div>}</div></CardContent></Card>; }
