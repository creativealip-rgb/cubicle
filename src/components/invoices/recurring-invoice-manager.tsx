...
export function RecurringInvoiceManager({ rules, clients, projects, canWrite, defaultCurrency }: Props) {
  ...
  return <section id="recurring-invoices" className="rounded-xl border bg-card">
    ...
    {rules.length === 0 ? <div className="flex min-h-48 flex-col items-center justify-center px-4 py-8 text-center"><span className="mb-3 rounded-full bg-muted p-3"><CalendarClock className="h-5 w-5 text-muted-foreground" /></span><p className="font-medium">{t("Belum ada invoice berulang", "No recurring invoices yet")}</p><p className="mt-1 max-w-md text-sm text-muted-foreground">{t("Otomatiskan tagihan rutin. Invoice hasil tetap berupa draft untuk diperiksa sebelum dikirim.", "Automate repeat billing. Generated invoices stay as drafts for review before sending.")}</p>{canWrite && clients.length > 0 ? <Button className="mt-4" size="sm" variant="outline" onClick={create}>{t("Buat invoice berulang", "Create recurring invoice")}</Button> : null}</div> : <div className="divide-y">{rules.map((rule) => {
    ...
    <Sheet open={open} onOpenChange={(next) => !pending && setOpen(next)}><SheetContent className="w-full overflow-y-auto p-0 sm:max-w-xl"><form onSubmit={submit} className="flex min-h-full flex-col"><SheetHeader className="border-b px-5 py-5"><SheetTitle>{editingId ? t("Edit invoice berulang", "Edit recurring invoice") : t("Invoice berulang baru", "New recurring invoice")}</SheetTitle><SheetDescription>{t("Atur klien, jadwal, dan detail draft invoice.", "Set client, schedule, and draft invoice details.")}</SheetDescription></SheetHeader><div className="grid flex-1 gap-4 px-5 py-5 sm:grid-cols-2">
    ...
    </div><SheetFooter className="sticky bottom-0 border-t bg-background px-5 py-4"><Button type="button" variant="outline" disabled={pending} onClick={() => setOpen(false)}>{t("Batal", "Cancel")}</Button><Button type="submit" disabled={pending}>{pending ? t("Menyimpan…", "Saving…") : t("Simpan invoice berulang", "Save recurring invoice")}</Button></SheetFooter></form></SheetContent></Sheet>
