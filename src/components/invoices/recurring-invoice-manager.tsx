...
export function RecurringInvoiceManager({ rules, clients, projects, canWrite, defaultCurrency }: Props) {
  ...
  return <section id="recurring-invoices" className="rounded-xl border bg-card">
    ...
    <Dialog open={open} onOpenChange={(next) => !pending && setOpen(next)}><DialogContent className="max-h-[90vh] overflow-y-auto p-0 sm:max-w-2xl"><form onSubmit={submit} className="flex flex-col"><DialogHeader className="border-b px-6 py-5 text-left"><DialogTitle>{editingId ? t("Edit invoice berulang", "Edit recurring invoice") : t("Invoice berulang baru", "New recurring invoice")}</DialogTitle><DialogDescription>{t("Atur klien, jadwal, dan detail draft invoice.", "Set client, schedule, and draft invoice details.")}</DialogDescription></DialogHeader><div className="grid gap-4 px-6 py-5 sm:grid-cols-2">
