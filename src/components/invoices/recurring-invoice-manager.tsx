...
export function RecurringInvoiceManager({ rules, clients, projects, canWrite, defaultCurrency }: Props) {
  ...
  return <section id="recurring-invoices" className="rounded-xl border bg-card">
    ...
    <Dialog open={open} onOpenChange={(next) => !pending && setOpen(next)}><DialogContent className="max-h-[90vh] overflow-hidden p-0 sm:max-w-2xl"><form onSubmit={submit} className="flex max-h-[90vh] flex-col"><DialogHeader className="shrink-0 border-b px-6 py-5 text-left"><DialogTitle>{editingId ? t("Edit invoice berulang", "Edit recurring invoice") : t("Invoice berulang baru", "New recurring invoice")}</DialogTitle><DialogDescription>{t("Atur klien, jadwal, dan detail draft invoice.", "Set client, schedule, and draft invoice details.")}</DialogDescription></DialogHeader><div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-6 py-5 sm:grid-cols-2">
      <div className="space-y-1.5"><Label>{t("Klien", "Client")}</Label><Select value={form.clientId} onValueChange={(value) => { set("clientId", value); set("projectId", ""); }}><SelectTrigger><SelectValue placeholder={t("Pilih klien", "Select client")} /></SelectTrigger><SelectContent>{clients.map((client) => <SelectItem key={client.id} value={client.id}>{client.name}</SelectItem>)}</SelectContent></Select></div>
      <div className="space-y-1.5"><Label>{t("Proyek (opsional)", "Project (optional)")}</Label><Select value={form.projectId || "none"} onValueChange={(value) => set("projectId", value === "none" ? "" : value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">—</SelectItem>{filteredProjects.map((project) => <SelectItem key={project.id} value={project.id}>{project.name}</SelectItem>)}</SelectContent></Select></div>
      <div className="space-y-1.5"><Label>{t("Frekuensi", "Frequency")}</Label><Select value={form.frequency} onValueChange={(value) => set("frequency", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="monthly">{t("Bulanan", "Monthly")}</SelectItem><SelectItem value="quarterly">{t("Kuartalan", "Quarterly")}</SelectItem><SelectItem value="yearly">{t("Tahunan", "Yearly")}</SelectItem></SelectContent></Select></div>
