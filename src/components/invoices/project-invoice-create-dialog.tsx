...
export function ProjectInvoiceCreateDialog({ project, client, baseCurrency, currencyRates = [], triggerLabel = "Buat Invoice" }: {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    <DialogTrigger asChild><Button size="sm" variant={triggerLabel === "Buat Invoice" ? "default" : "outline"} className="gap-1"><Plus className="h-4 w-4" /> {triggerLabel}</Button></DialogTrigger>
