...
export function PromptResult({ result, loading, onEdit, onRegenerate }: { result: PromptGenerationResult | null; loading: boolean; onEdit(): void; onRegenerate(): void }) {
  ...
  return <div className="min-h-[220px] rounded-xl border bg-white p-3.5 sm:p-4">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-medium uppercase tracking-wide text-primary">Hasil</p><h2 className="mt-0.5 text-base font-semibold">{result.title}</h2></div><div className="flex gap-1.5"><Button variant="outline" size="sm" className="h-7 px-2.5 text-xs" onClick={onEdit}><Pencil className="h-3.5 w-3.5"/>Edit brief</Button><Button variant="outline" size="sm" className="h-7 px-2.5 text-xs" onClick={onRegenerate}><RotateCcw className="h-3.5 w-3.5"/>Generate ulang</Button></div></div>
    ...
    <Button size="sm" className="mt-2.5 h-8 text-xs" onClick={() => copy(rawJson)}>{copied ? <Check className="h-3.5 w-3.5"/> : <Copy className="h-3.5 w-3.5"/>}Copy JSON</Button>
  </div>;
}
