    ...
    <DialogContent className="sm:max-w-lg">
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="manual-time-project">Project *</Label>
          <Select value={projectId} onValueChange={(value) => { setProjectId(value); setTaskId(""); }}>
            <SelectTrigger id="manual-time-project"><SelectValue placeholder="Pilih project" /></SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select value={taskId} onValueChange={setTaskId} disabled={!projectId}>
            <SelectTrigger id="manual-time-task"><SelectValue placeholder="Pilih task" /></SelectTrigger>
            <SelectContent>{projectTasks.map((task) => <SelectItem key={task.id} value={task.id}>{task.title}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div className="space-y-2">
          <Label htmlFor="manual-time-description">Deskripsi *</Label>
          <Input id="manual-time-description" value={description} onChange={(event) => setDescription(event.target.value)} required />
        </div>
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-2"><Label htmlFor="manual-time-date">Tanggal</Label><Input id="manual-time-date" type="date" value={date} onChange={(event) => setDate(event.target.value)} required /></div>
          <div className="space-y-2"><Label htmlFor="manual-time-hours">Jam</Label><Input id="manual-time-hours" type="number" min="0" value={hours} onChange={(event) => setHours(event.target.value)} /></div>
          <div className="space-y-2"><Label htmlFor="manual-time-minutes">Menit</Label><Input id="manual-time-minutes" type="number" min="0" max="59" value={minutes} onChange={(event) => setMinutes(event.target.value)} /></div>
        </div>
