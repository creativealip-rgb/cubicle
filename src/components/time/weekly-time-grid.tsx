...
function WeeklyDraftRow({ index, dates, projects, tasks, clients, canWrite, onSaved, mobile = false }: { index: number; dates: Date[]; projects: ProjectOption[]; tasks: TaskOption[]; clients: Array<{ id: string; name: string }>; canWrite: boolean; onSaved: () => void; mobile?: boolean }) {
  ...
  if (mobile) return <div className="space-y-3 rounded-lg border p-3"><div className="grid gap-2 sm:grid-cols-2">{projectPicker}{taskPicker}</div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{dates.map((date) => <label key={date.toISOString()} className="text-xs text-muted-foreground"><span className="mb-1 block">{date.toLocaleDateString(locale,{weekday:"short",day:"numeric",timeZone:"UTC"})}</span><Input placeholder="HH:MM" disabled={!canWrite || pending || !projectId || !taskId} className="h-9 text-center text-xs" onBlur={(event) => save(date,event.target.value)}/></label>)}</div></div>;
          ...
          <>
            <div className="hidden overflow-x-auto lg:block">
              <div className="min-w-[1092px]">
                ...
                <div className="grid grid-cols-[minmax(500px,3.05fr)_repeat(7,minmax(72px,.55fr))_80px] items-center gap-1 bg-muted/30 py-2.5 text-xs font-semibold"><span>{t("Total Harian", "Daily Total")}</span>{dates.map((date) => <span key={date.toISOString()} className="text-center">-</span>)}<span className="text-center">00:00</span></div>
              </div>
            </div>
