...
export async function TimeRouteContent({ mode, view = "daily", selectedDate = localDateIso(new Date()), action }: { mode: "timer" | "timesheet" | "history" | "approvals"; view?: "daily" | "weekly"; selectedDate?: string; action?: string }) {
  ...
  const primaryActions = canWrite ? (
    <>
      ...
      {!activeTimer && <NewTimerDialog initialOpen={action === "timer"} workspaceId={workspaceId} projects={writableProjectList.map((p) => ({ id: p.id, name: p.name, customerRef: p.clientId }))} tasks={writableTaskList.map((t) => ({ id: t.id, title: t.title, projectRef: t.projectId }))} />}
    </>
  ) : null;
      ...
      {mode === "timer" && (
        <>
          ...
          <section aria-labelledby="recent-time-heading">
            ...
            <Timesheet entries={entries.slice(0, 3).map((e) => ({ id: e.id, description: e.description, tags: e.tags, durationMinutes: e.durationMinutes, manualMinutes: e.manualMinutes, billable: e.billable ?? false, hourlyRate: e.hourlyRate, startTime: e.startTime, endTime: e.endTime, status: e.status, clientId: e.clientId, projectId: e.projectId, activityId: e.activityId, taskId: e.taskId, clientName: e.clientName, projectName: e.projectName, activityName: e.activityName, projectCurrency: e.projectCurrency, projectTimeTrackingMode: e.projectTimeTrackingMode, billingType: projectList.find((project) => project.id === e.projectId)?.billingType, taskTitle: e.taskTitle, userName: e.userName, createdAt: e.createdAt }))} clients={clientList} projects={projectList} tasks={taskList} activities={activityList} />
          </section>
        </>
      )}
      {mode === "timesheet" && (
