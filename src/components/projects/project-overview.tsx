...
import { formatMinutesCompact, getProjectOverviewBilling } from "@/lib/project-overview-billing";

export function ProjectOverview({ project, progress, taskUsageProgress, trackedMinutes, retainerUsedMinutes, billableAmount, invoicedAmount, outstandingAmount, retainerPeriod, recentTime, recentInvoices, recentFiles, editAction, billingEditAction, locale, t }: any) {
  ...
  const configuredAmount = overviewBilling.configuredAmount;
  const billingSettingAmount = Number(isRetainer ? project.retainerFee : isHourly ? project.rate : project.budget) || 0;
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <Card className="h-full rounded-xl"><CardContent className="flex h-full flex-col p-4"><h3 className="mb-4 text-sm font-bold">{t("Detail Proyek", "Project Details")}</h3><dl className="space-y-3 pb-4 text-sm"><div><dt className="text-xs text-muted-foreground">Client</dt><dd className="font-medium">{project.clientName || t("Tanpa klien", "No client")}</dd></div><div><dt className="text-xs text-muted-foreground">{t("Dibuat", "Created")}</dt><dd>{new Date(project.createdAt).toLocaleDateString(locale)}</dd></div><div><dt className="text-xs text-muted-foreground">{t("Jatuh tempo", "Due date")}</dt><dd>{project.dueDate ? new Date(project.dueDate).toLocaleDateString(locale) : "—"}</dd></div><div><dt className="text-xs text-muted-foreground">Status</dt><dd><Badge variant="outline">{project.status}</Badge></dd></div><div><dt className="text-xs text-muted-foreground">{t("Deskripsi", "Description")}</dt><dd>{project.description || t("Tidak ada deskripsi", "No description")}</dd></div></dl><div className="mt-auto border-t pt-3">{editAction}</div></CardContent></Card>
      <Card className="h-full rounded-xl"><CardContent className="flex h-full flex-col p-4"><h3 className="mb-4 text-sm font-bold">{t("Pengaturan Billing", "Billing Settings")}</h3><dl className="space-y-3 text-sm"><div><dt className="text-xs text-muted-foreground">{t("Model billing", "Billing model")}</dt><dd className="font-medium">{billingLabel}</dd></div><div><dt className="text-xs text-muted-foreground">{isRetainer ? t("Biaya bulanan", "Monthly fee") : isHourly ? t("Tarif per jam", "Hourly rate") : t("Nilai proyek", "Project value")}</dt><dd className="font-mono font-bold">{billingSettingAmount ? `${formatMoney(billingSettingAmount, project.currency)}${isHourly ? `/${t("jam", "hr")}` : ""}` : t("Belum diatur", "Not set")}</dd></div><div><dt className="text-xs text-muted-foreground">Currency</dt><dd>{project.currency}</dd></div>{isRetainer && <div><dt className="text-xs text-muted-foreground">{t("Jam termasuk", "Included hours")}</dt><dd>{Math.round(Number(project.retainerIncludedMinutes || 0) / 60)}h</dd></div>}</dl><div className="mt-auto border-t pt-3">{billingEditAction}</div></CardContent></Card>
      <Card className="h-full rounded-xl"><CardContent className="flex h-full flex-col p-4"><h3 className="mb-4 text-sm font-bold">{progressTitle}</h3><p className="font-mono text-xl font-bold">{configuredAmount || isRetainer || isHourly ? progressLead : "—"}</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-muted"><div className={`h-full ${progressPercent >= 90 ? "bg-rose-500" : progressPercent >= 70 ? "bg-amber-500" : "bg-primary"}`} style={{width:`${progressPercent}%`}} /></div><div className="mt-2 flex justify-between text-xs text-muted-foreground"><span>{progressPercent}% {isRetainer ? t("terpakai", "used") : t("ditagih", "invoiced")}</span><span>{progressRest}</span></div><div className="mt-auto border-t pt-3"><Link href="?tab=billing" className="text-xs font-medium text-primary hover:underline">{t("Kelola invoice", "Manage invoices")}</Link></div></CardContent></Card>
    </div>
    <div className="grid gap-4 lg:grid-cols-3">
