...
function SectionBody({ section, accent, panel, buttonRadius }: { section: PersonalSiteSection; accent: string; panel: string; buttonRadius: string }) {
  ...
  switch (section.type) {
    ...
    case "portfolio":
      return <div className="grid gap-4 md:grid-cols-2">{section.projects.filter((project) => project.title || project.description || project.url).map((project) => (
        <ItemCard key={project.id} panel={panel}>{marker}<h3 className="text-lg font-semibold">{project.title}</h3>{project.description && <p className="mt-2 whitespace-pre-wrap text-sm leading-6 opacity-70">{project.description}</p>}{project.url && <a className="mt-4 inline-flex min-h-10 items-center font-semibold underline underline-offset-4" href={safePublicHref(project.url)} target="_blank" rel="noreferrer">Open project</a>}</ItemCard>
      ))}</div>;
    case "testimonials":
      return <div className="grid gap-4 md:grid-cols-2">{section.testimonials.filter((item) => item.quote && (item.author || item.role)).map((item) => (
    ...
    case "contact":
      return <div className="flex flex-wrap gap-3">{section.methods.filter((item) => item.label && (item.value || item.url)).map((item) => item.url ? (
    ...
    case "gallery":
      return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{section.images.filter((img) => img.url).map((img) => (
        <div key={img.id} className="overflow-hidden rounded-2xl"><Image src={img.url.startsWith("http") ? img.url : img.url.startsWith("/") ? img.url : `/${img.url}`} alt={img.alt || ""} width={600} height={450} className="aspect-[4/3] w-full object-cover" loading="lazy" /></div>
      ))}</div>;
    case "embed":
    ...
    <main data-testid="personal-site-renderer" data-theme={site.theme} style={accentStyle} className={`${embedded ? "min-h-0" : "min-h-screen"} overflow-hidden ${styles.page}`}>
      <section className={`relative ${styles.hero} ${heroShellClass} px-6 py-14 sm:px-10 sm:py-20 lg:px-16 lg:py-24`} style={{ backgroundColor: themeConfig?.headerStyle === "minimal" ? "transparent" : accent }}>
        ...
        {visiblePages.length > 1 && (
          <nav className="mt-8 flex flex-wrap gap-2" aria-label="Site pages">
      ...
      {visibleSections.map((section, index) => (
        <AnimateOnScroll key={section.id} animation={section.animation}>
          <section data-section-type={section.type} className={`${index % 2 === 0 ? styles.sectionAlt : ""} px-6 py-14 sm:px-10 lg:px-16 lg:py-20`}>
            <div className="mx-auto max-w-6xl"><h2 className="mb-7 text-2xl font-bold tracking-[-0.01em] sm:text-3xl" style={headingStyle}>{section.heading}</h2><SectionBody section={section} accent={accent} panel={styles.panel} buttonRadius={buttonRadius} /></div>
          </section>
        </AnimateOnScroll>
      ))}
