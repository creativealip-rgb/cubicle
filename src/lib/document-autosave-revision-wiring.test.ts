...
describe("autosave revision/stale-write protection wiring", () => {
  ...
  it("edit pages load the revision and the editor passes it back on every save", () => {
    ...
    expect(proposalEditPage).toMatch(/async function saveBlocks\(next: Parameters<typeof saveProposalBlocks>\[1\]\[\"contentBlocks\"\], revision: number\)/);
    ...
    expect(contractEditPage).toMatch(/async function saveBlocks\(next: Parameters<typeof saveContractBlocks>\[1\]\[\"contentBlocks\"\], revision: number\)/);
