...
describe("client and project detail UX", () => {
  ...
  it("keeps client summary at two columns and portal in lower tabs", () => {
    expect(clientDetail).toContain('className="grid grid-cols-1 gap-4 sm:grid-cols-2"');
    expect(clientDetail).not.toContain('<p className="text-xs text-muted-foreground">Portal</p>');
  ...
  it("shows nominal without fixed-rate wording and removes services tab", () => {
    ...
    expect(projectDetail).toContain('t("Jenis Pekerjaan", "Work Types")');
  });
