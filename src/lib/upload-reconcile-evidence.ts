...
export const UPLOAD_EVIDENCE_CHECKS = ["database_ledger_complete", "object_pagination_complete", "object_references_complete", "object_metadata_verified", "orphan_grace_applied", "bounded_orphan_repair"] as const;
...
export function buildUploadReconcileEvidence(input: { dbReport: UploadReconcileReport; objects: Objects; bucket: string; prefixes: string[]; referencesScanned: number }) {
  ...
  if (input.objects.deletedObjects < 0 || input.objects.deletedObjects > 100 || input.dbReport.actions.deletedObjects !== 0) throw new Error("UPLOAD_RECONCILE_DELETE_BOUND_EXCEEDED");
  return {
    version: 1 as const,
    complete: true,
