...
describe("reusable task editing", () => {
  it("opens edit dialog with shared form and locked reusable mode", () => {
    expect(workspace).toContain("TaskForm");
    expect(workspace).toContain('taskMode="reusable"');
    expect(workspace).toContain("Ubah");
  });
  ...
  it("keeps lifecycle explicit and server mode immutable", () => {
    expect(workspace).toContain("Arsipkan");
    expect(workspace).toContain("Pulihkan");
