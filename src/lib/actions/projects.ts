...
export async function createProject(input: z.input<typeof projectCreateSchema>) {
  ...
  const [project] = await db.insert(projects).values({
    ...
    retainerFee: parsed.retainerFee != null ? String(parsed.retainerFee) : null,
    retainerIncludedMinutes: parsed.retainerIncludedMinutes ?? null,
    retainerPeriodUnit: parsed.billingModel === "retainer" ? "month" : null,
    retainerResetDay: parsed.retainerResetDay ?? null,
    retainerOveragePolicy: parsed.retainerOveragePolicy ?? null,
    retainerOverageRate: parsed.retainerOverageRate != null ? String(parsed.retainerOverageRate) : null,
    timeTrackingMode,
