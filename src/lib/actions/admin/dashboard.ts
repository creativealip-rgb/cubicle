...
export type GrowthDashboard = {
  ...
  trends: Array<{ bucket: string; visitors: number; signupStarts: number; signupCompletions: number; activated: number; revenue: number }>;
  acquisition: Array<{ source: string; campaign: string; visitors: number; signupCompletions: number; spend: number; conversionRate: number | null; cac: number | null }>;
};
...
export async function getAdminGrowthDashboard(input?: string): Promise<GrowthDashboard> {
  ...
  const result = await db.execute(sql`
    WITH bounds AS (
      ...
      SELECT
        ...
        (SELECT count(*) FROM activation_milestones m, bounds b WHERE m.first_client_at >= b.start AND m.first_client_at < b.finish)::int AS first_client_count,
        (SELECT count(*) FROM activation_milestones m, bounds b WHERE m.first_project_at >= b.start AND m.first_project_at < b.finish)::int AS first_project_count,
        (SELECT count(*) FROM (SELECT workspace_id, min(created_at) AS first_at FROM tasks GROUP BY workspace_id) x, bounds b WHERE x.first_at >= b.start AND x.first_at < b.finish)::int AS first_task_count,
        (SELECT count(*) FROM (SELECT workspace_id, min(created_at) AS first_at FROM invoices GROUP BY workspace_id) x, bounds b WHERE x.first_at >= b.start AND x.first_at < b.finish)::int AS first_invoice_count,
        (SELECT count(*) FROM (SELECT workspace_id, min(visited_at) AS first_at FROM portal_visits GROUP BY workspace_id) x, bounds b WHERE x.first_at >= b.start AND x.first_at < b.finish)::int AS first_portal_count,
  ...
  const bucket = range === "12m" ? sql`'month'` : sql`'day'`;
  const detail = await db.execute(sql`WITH bounds AS (SELECT now() AS finish, now() - ${interval} AS start), buckets AS (SELECT generate_series(date_trunc(${bucket}, (SELECT start FROM bounds)), date_trunc(${bucket}, (SELECT finish FROM bounds) - interval '1 microsecond'), CASE WHEN ${range === "12m"} THEN interval '1 month' ELSE interval '1 day' END) AS bucket), events AS (SELECT date_trunc(${bucket}, occurred_at) AS bucket, count(DISTINCT anonymous_id) FILTER (WHERE event_name='landing_viewed') visitors, count(*) FILTER (WHERE event_name='signup_started') signup_starts, count(*) FILTER (WHERE event_name='signup_completed') signup_completions FROM analytics_events, bounds WHERE occurred_at >= start AND occurred_at < finish GROUP BY 1), milestones AS (SELECT w.id, (SELECT min(created_at) FROM clients WHERE workspace_id=w.id) first_client_at, (SELECT min(created_at) FROM projects WHERE workspace_id=w.id) first_project_at, (SELECT min(ts) FROM (SELECT created_at ts FROM tasks WHERE workspace_id=w.id UNION ALL SELECT created_at FROM invoices WHERE workspace_id=w.id UNION ALL SELECT created_at FROM time_entries WHERE workspace_id=w.id UNION ALL SELECT visited_at FROM portal_visits WHERE workspace_id=w.id) q) first_activity_at FROM workspaces w), activation_dates AS (SELECT id, GREATEST(first_client_at, first_project_at, first_activity_at) activated_at FROM milestones WHERE first_client_at IS NOT NULL AND first_project_at IS NOT NULL AND first_activity_at IS NOT NULL), activations AS (SELECT date_trunc(${bucket}, activated_at) bucket, count(*)::int activated FROM activation_dates, bounds WHERE activated_at >= start AND activated_at < finish GROUP BY 1), revenue AS (SELECT date_trunc(${bucket}, COALESCE(paid_at,created_at)) AS bucket, COALESCE(sum(amount),0) revenue FROM pakasir_payments, bounds WHERE status='completed' AND payment_type='plan' AND COALESCE(paid_at,created_at) >= start AND COALESCE(paid_at,created_at) < finish GROUP BY 1) SELECT b.bucket::text, COALESCE(e.visitors,0)::int visitors, COALESCE(e.signup_starts,0)::int signup_starts, COALESCE(e.signup_completions,0)::int signup_completions, COALESCE(a.activated,0)::int activated, COALESCE(rv.revenue,0)::numeric revenue FROM buckets b LEFT JOIN events e USING(bucket) LEFT JOIN activations a USING(bucket) LEFT JOIN revenue rv USING(bucket) ORDER BY b.bucket`);
  const acquisitionResult = await db.execute(sql`WITH bounds AS (SELECT now() AS finish, now() - ${interval} AS start), a AS (SELECT COALESCE(source, 'direct / unknown') source, COALESCE(campaign, '—') campaign, count(DISTINCT anonymous_id) FILTER (WHERE event_name='landing_viewed') visitors, count(*) FILTER (WHERE event_name='signup_completed') signup_completions FROM analytics_events, bounds WHERE occurred_at >= start AND occurred_at < finish GROUP BY 1,2), s AS (SELECT COALESCE(source, 'direct / unknown') source, COALESCE(campaign, '—') campaign, COALESCE(sum(amount),0) spend FROM marketing_spend, bounds WHERE spend_date >= start::date AND spend_date < finish::date AND currency='IDR' GROUP BY 1,2) SELECT COALESCE(a.source,s.source) source, COALESCE(a.campaign,s.campaign) campaign, COALESCE(a.visitors,0)::int visitors, COALESCE(a.signup_completions,0)::int signup_completions, COALESCE(s.spend,0)::numeric spend FROM a FULL JOIN s USING(source,campaign) ORDER BY signup_completions DESC, spend DESC LIMIT 20`);
  const trends = detail.rows.map((x) => ({ bucket: String(x.bucket), visitors: Number(x.visitors), signupStarts: Number(x.signup_starts), signupCompletions: Number(x.signup_completions), activated: Number(x.activated), revenue: Number(x.revenue) }));
  const acquisition = acquisitionResult.rows.map((x) => { const visitors = Number(x.visitors), completions = Number(x.signup_completions), spend = Number(x.spend); return { source: String(x.source), campaign: String(x.campaign), visitors, signupCompletions: completions, spend, conversionRate: visitors ? completions / visitors * 100 : null, cac: completions ? spend / completions : null }; });
