...
describe("time history editor route",()=>{
 ...
 it("keeps row editor, pagination, blank-row prompt, and delete controls",()=>{const s=read("src/components/time/timesheet.tsx");expect(s).toContain("PAGE_SIZE = 10");expect(s).toContain("cursor-pointer");expect(s).toContain("openEdit(entry)");expect(s).toContain("Tambah proyek, task, dan detail lainnya");expect(s).toContain("deleteTimeEntry")});
});
