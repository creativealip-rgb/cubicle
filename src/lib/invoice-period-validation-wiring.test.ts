...
describe("timesheet period validation UX",()=>{
 ...
 it("shows inline error and disables submit",()=>{expect(source).toContain('const incompleteTimesheetPeriods');expect(source).toContain('Lengkapi periode timesheet');expect(source).toContain('disabled={loading || incompleteTimesheetPeriods || invoiceTotal <= 0}');});
});
