...
describe("MH3 time reporting",()=>{
 ...
 it("groups by client project task and member",()=>{const r=buildTimeReport(entries);expect(r.byClient).toHaveLength(1);expect(r.byProject[0]).toMatchObject({id:"p1",minutes:180,billableMinutes:120,value:200000});expect(r.byTask).toHaveLength(3);expect(r.byMember[0]).toMatchObject({id:"u1",minutes:150,value:230000});});
});
