...
describe("weekly time grid", () => {
  ...
  it("groups by project and activity while preserving related tasks in totals", () => {
    ...
    const activityRow = grid.rows.find((row) => row.activityId === "activity-1");
    expect(activityRow?.cells[0]).toMatchObject({ totalMinutes: 180, editableMinutes: 120, immutableMinutes: 60 });
    expect(activityRow?.totalMinutes).toBe(180);
