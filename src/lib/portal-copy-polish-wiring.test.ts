...
describe("portal copy polish",()=>{
 ...
 it("makes meeting primary and report secondary",()=>{const actions=read("src/components/portal/portal-action-buttons.tsx");expect(actions).toContain('variant="outline"');expect(actions).toContain('onClick={() => setKind("meeting")}');expect(actions).toContain('onClick={() => setKind("report")}');});
});
