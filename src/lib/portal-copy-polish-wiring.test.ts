...
describe("portal copy polish",()=>{
 ...
 it("keeps meeting request as the only portal request action",()=>{const actions=read("src/components/portal/portal-action-buttons.tsx");expect(actions).toContain('onClick={() => setKind("meeting")}');expect(actions).not.toContain('setKind("report")');});
});
