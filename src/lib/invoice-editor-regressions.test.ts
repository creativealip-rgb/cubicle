...
describe("invoice editor regressions",()=>{
  ...
  it("exposes full-form client choices as stable accessible options",()=>{const s=read("src/components/forms/invoice-form.tsx"); expect(s).toContain('role="option"'); expect(s).toContain("aria-selected={form.clientId === client.id}");});
});
