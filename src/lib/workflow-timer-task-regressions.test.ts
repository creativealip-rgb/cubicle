...
describe("client project task timer workflow",()=>{
  ...
  it("accepts active workflow tasks at server boundary",()=>{const s=read("src/lib/time-task-eligibility-db.ts");expect(s).not.toContain('eq(tasks.mode, "reusable")');expect(s).toContain('eq(tasks.lifecycle, "active")');});
