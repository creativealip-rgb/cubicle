...
export async function scanUploadObjectInventory(client: ObjectClient, input: { bucket: string; now: Date; graceMs: number; references: Reference[] }) {
  ...
  let pages = 0;
  for (const prefix of ["quarantine/", "workspaces/"]) {
    let token: string | undefined;
    do {
      const page = await client.send(new ListObjectsV2Command({ Bucket: input.bucket, Prefix: prefix, ContinuationToken: token }));
      pages += 1;
