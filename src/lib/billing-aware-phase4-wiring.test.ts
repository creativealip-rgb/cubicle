...
describe("billing-aware Phase 4 Project Task UX",()=>{
 ...
 it("derives canonical task mode and exposes mode-aware form",()=>{const a=read("src/lib/actions/tasks.ts");expect(a).toContain("resolveProjectTaskMode");expect(a).toContain("mode: projectMode");expect(a).toContain('behavior: projectMode === "workflow"');const f=read("src/components/forms/task-form.tsx");expect(f).toContain("taskMode")});
});
