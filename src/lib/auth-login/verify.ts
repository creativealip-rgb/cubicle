...
export type OtpVerifyDeps = {
  ...
  lockChallenge(flowId: string): Promise<OtpChallenge | null>;
  failAttempt(id: string): Promise<void>;
...
export async function runOtpVerify(
  flowId: string,
  code: string,
  deps: OtpVerifyDeps,
) {
  if (!flowId || !/^\d{6}$/.test(code)) return invalid;
  if (!(await deps.rateLimit(flowId)))
    return {
      ok: false as const,
      code: "RATE_LIMITED",
  ...
  if (
    ...
    return invalid;
  if (!verifyVerifier(code, challenge.codeHash, deps.secret)) {
    await deps.failAttempt(challenge.id);
    return invalid;
  }
  ...
  return {
    ...
    trustedCookie: `${trusted.id}.${trusted.token}`,
  };
}
