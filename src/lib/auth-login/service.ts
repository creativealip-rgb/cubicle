...
export const loginInput = z.object({
  ...
  trustedCookie: z.string().optional(),
});
...
export function validateLoginInput(input: unknown) {
  ...
  return parsed.success
    ? {
        ...
        value: {
          ...parsed.data,
          email: normalizeLoginEmail(parsed.data.email),
        },
      }
    : { ok: false as const };
}
export function sanitizeCallbackUrl(value: unknown) {
...
export function parseTrustedCookie(value?: string) {
  if (!value) return null;
  const dot = value.indexOf(".");
  const id = value.slice(0, dot);
  const token = value.slice(dot + 1);
  return /^[0-9a-f-]{36}$/i.test(id) && token.length >= 8
    ? { id, token }
    : null;
}
export function isValidTrustedDevice(
  input: {
    userId: string;
    deviceUserId: string;
    tokenValid: boolean;
...
export function createOtpChallenge(secret: string, now = new Date()) {
  const code = generateOtp();
  return {
    code,
    codeHash: hashVerifier(code, secret),
    expiresAt: new Date(now.getTime() + OTP_EXPIRY_MS),
    resendAfter: new Date(now.getTime() + 60_000),
  };
}
export function createTrustedDevice(secret: string, now = new Date()) {
  const token = generateToken();
  return {
    token,
    tokenHash: hashVerifier(token, secret),
    expiresAt: new Date(now.getTime() + TRUSTED_DEVICE_EXPIRY_MS),
  };
}
...
export async function runPasswordLoginStart(
  ...
  if (!(await deps.verifyPassword(credential.passwordHash, input.password)))
    return {
      ok: false as const,
      code: "INVALID_CREDENTIALS",
      error: INVALID_CREDENTIALS,
  ...
  if (
    ...
    isValidTrustedDevice(
      {
        ...
        tokenValid: verifyVerifier(cookie.token, device.tokenHash, deps.secret),
    ...
    return {
      ...
      redirectTo: sanitizeCallbackUrl(input.callbackUrl),
    };
  }
  ...
  try {
    ...
    await deps.deleteChallenge(flowId);
    return {
      ok: false as const,
  ...
  return {
    ...
    maskedEmail: maskEmail(credential.email),
