...
describe("password email OTP login state machine", () => {
  it("keeps password pending until OTP succeeds", () => {
    ...
    expect(
      advanceLogin(
        state,
        { type: "password_verified", trusted: false },
  ...
  it("bypasses OTP for valid trusted device", () => {
    ...
    expect(
      advanceLogin(
        state,
        { type: "password_verified", trusted: true },
        new Date("2026-01-01T00:00:00Z"),
      ),
    ).toEqual({ status: "authenticated", userId: "u1" });
  });
  it("authenticates valid OTP and rejects expired OTP", () => {
    ...
    ).toEqual({ status: "password_pending", userId: "u1" });
  });
  it("does not enforce legacy twoFactorEnabled", () => {
    ...
    expect(
      advanceLogin(
        state,
        { type: "password_verified", trusted: false, twoFactorEnabled: true },
        new Date("2026-01-01T00:00:00Z"),
      ).status,
    ).toBe("otp_pending");
  });
  it("uses shared OTP expiry contract", () => {
    ...
    expect(
      advanceLogin(
        { status: "password_pending", userId: "u1" },
        { type: "password_verified", trusted: false },
        now,
      ),
    ).toMatchObject({
