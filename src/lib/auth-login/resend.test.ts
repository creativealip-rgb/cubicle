...
const deps = (overrides: Partial<OtpResendDeps> = {}): OtpResendDeps => ({
  ...
  findChallenge: vi
    ...
    .mockResolvedValue({
      id: "c1",
      email: "u@example.com",
      resendAfter: new Date("2026-09-06T23:59:00Z"),
      consumedAt: null,
    }),
  replaceCode: vi
    .fn()
...
describe("OTP resend", () => {
        ...
        deps({
          findChallenge: vi
            ...
            .mockResolvedValue({
              id: "c1",
              email: "u@example.com",
              resendAfter: new Date("2026-09-07T00:01:00Z"),
              consumedAt: null,
            }),
        }),
      ),
    ).resolves.toMatchObject({ ok: false, code: "COOLDOWN" });
    await expect(
      runOtpResend(
        "flow",
  ...
  it("replaces code and returns public expiry", async () => {
    const d = deps();
    await expect(runOtpResend("flow", d)).resolves.toEqual({
      ok: true,
      expiresAt: "2026-09-07T00:10:00.000Z",
    });
    expect(d.replaceCode).toHaveBeenCalledWith("c1");
  ...
  it("rolls back replacement on delivery failure", async () => {
    ...
    await expect(runOtpResend("flow", d)).resolves.toMatchObject({
      ok: false,
      code: "DELIVERY_FAILED",
    });
    expect(d.rollback).toHaveBeenCalledWith("c1");
  });
});
