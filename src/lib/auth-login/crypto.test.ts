...
} from "@/lib/auth-login/crypto";

describe("auth login crypto", () => {
  it("generates six digit OTP and random token", () => {
    expect(generateOtp()).toMatch(/^\d{6}$/);
    expect(generateToken()).toMatch(/^[A-Za-z0-9_-]{32,}$/);
  });
  it("uses secret-bound timing-safe verifier", () => {
    const hash = hashVerifier("code", "secret");
    expect(verifyVerifier("code", hash, "secret")).toBe(true);
    expect(verifyVerifier("code", hash, "wrong")).toBe(false);
  });
  ...
  it("keeps OTP range and token uniqueness feasible", () => {
    ...
    expect(otps.every((otp) => Number(otp) >= 0 && Number(otp) <= 999999)).toBe(
      true,
    );
  ...
  it("domain-separates verifier and hashes deterministically", () => {
    ...
    expect(hashVerifier("code", "secret")).not.toBe(
      hashVerifier("other", "secret"),
    );
  });
  it("masks email and exposes required expiries", () => {
    expect(maskEmail("alice@example.com")).toBe("a***e@example.com");
    expect(OTP_EXPIRY_MS).toBe(600000);
    expect(TRUSTED_DEVICE_EXPIRY_MS).toBe(2592000000);
  });
  it("defensively masks malformed and short email input", () => {
