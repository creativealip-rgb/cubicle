...
export const analyticsEvents = pgTable("analytics_events", {
  id: uuid("id").defaultRandom().primaryKey(), eventName: text("event_name").notNull(), occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull().defaultNow(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), anonymousId: text("anonymous_id"), sessionId: text("session_id"), userId: text("user_id").references(() => users.id, { onDelete: "set null" }), workspaceId: uuid("workspace_id").references(() => workspaces.id, { onDelete: "set null" }), source: text("source"), medium: text("medium"), campaign: text("campaign"), term: text("term"), content: text("content"), referrer: text("referrer"), referralId: text("referral_id"), metadata: jsonb("metadata").notNull().default(sql`'{}'::jsonb`),
});
export const subscriptionEvents = pgTable("subscription_events", {
  id: uuid("id").defaultRandom().primaryKey(), userId: text("user_id").references(() => users.id, { onDelete: "set null" }), workspaceId: uuid("workspace_id").references(() => workspaces.id, { onDelete: "set null" }), eventType: text("event_type").notNull(), fromPlan: text("from_plan"), toPlan: text("to_plan"), amount: numeric("amount", { precision: 12, scale: 2 }), billingPeriod: text("billing_period"), providerOrderId: text("provider_order_id"), occurredAt: timestamp("occurred_at", { withTimezone: true }).notNull().defaultNow(), metadata: jsonb("metadata").notNull().default(sql`'{}'::jsonb`),
...
export const uploadIntents = pgTable("upload_intents", {
  ...
  fileType: text("file_type", { enum: ["working_file", "deliverable"] }),
  clientId: uuid("client_id"),
  projectId: uuid("project_id"),
  folderId: uuid("folder_id"),
  intendedUploadedBy: text("uploaded_by"),
...
export const contractTemplates = pgTable(
  ...
  (table) => [
    uniqueIndex("contract_templates_one_default_per_ws_uidx")
...
export const proposalTemplates = pgTable(
  ...
  (table) => [
    uniqueIndex("proposal_templates_one_default_per_ws_uidx")
      .on(table.workspaceId)
      .where(sql`${table.isDefault} = true`),
  ],
);
