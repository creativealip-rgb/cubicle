...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!limit.allowed) return NextResponse.redirect(new URL(`/client-portal/${slug}?error=rate`, request.url), 303);
  ...
  if (!valid) return NextResponse.redirect(new URL(`/client-portal/${slug}?error=invalid`, request.url), 303);
  const secret = process.env.BETTER_AUTH_SECRET;
  if (!secret) return new NextResponse("Portal unavailable", { status: 503 });
  const response = NextResponse.redirect(new URL(`/client-portal/${slug}`, request.url), 303);
