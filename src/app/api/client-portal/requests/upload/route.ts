...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // File insert, request status flip, and workspace quota accounting all
    // commit in one transaction via withWorkspaceQuotaReservation (reserve ->
    // work -> consume). The old standalone reserve/consume pair left the
    // reservation stuck forever if the process died after the tx committed.
    // R2 stays outside the tx — on any failure the object is removed in the
    // catch below.
    const fileRow = await withWorkspaceQuotaReservation(client.workspaceId, upload.size, async (tx) => {
