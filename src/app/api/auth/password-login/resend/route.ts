...
export async function POST(request: Request) {
  ...
  const result = await runOtpResend(flowId, {
    ...
    findChallenge: async (flow) => {
      const [row] = await db
        .select({
          id: authLoginOtpChallenges.id,
          email: users.email,
          resendAfter: authLoginOtpChallenges.resendAfter,
          consumedAt: authLoginOtpChallenges.consumedAt,
        })
        .from(authLoginOtpChallenges)
        .innerJoin(users, eq(users.id, authLoginOtpChallenges.userId))
    ...
    replaceCode: async (id) => {
      ...
      await db
        ...
        .set({
          ...
          resendAfter: next.resendAfter,
          consumedAt: null,
        })
        .where(eq(authLoginOtpChallenges.id, id));
