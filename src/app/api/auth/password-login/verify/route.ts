...
export async function POST(request: Request) {
  ...
  if (process.env.PASSWORD_EMAIL_OTP_LOGIN_ENABLED !== "true")
  ...
  const flowId =
    request.headers
      .get("cookie")
      ?.match(/(?:^|; )cubiqlo\.login_flow=([^;]+)/)?.[1] ?? "";
  const sessionCookie: {
    current: Awaited<ReturnType<typeof createBetterAuthSession>> | null;
  } = { current: null };
  const result = await runOtpVerify(flowId, code, {
    secret,
    now: () => new Date(),
    rateLimit: async (flow) =>
      (
        await enforceRateLimit(
          request,
          "password-login-verify",
          { limit: 5, windowSec: 600 },
          { identity: flow || "missing", failureMode: "closed" },
        )
      ).allowed,
    lockChallenge: async (flow) =>
      (
        await db
          .select()
          .from(authLoginOtpChallenges)
          .where(eq(authLoginOtpChallenges.flowId, flow))
          .limit(1)
      )[0] ?? null,
    failAttempt: async (id) => {
      await db
        .update(authLoginOtpChallenges)
        .set({ attempts: sql`${authLoginOtpChallenges.attempts} + 1` })
        .where(
          and(
            eq(authLoginOtpChallenges.id, id),
            isNull(authLoginOtpChallenges.consumedAt),
            lt(authLoginOtpChallenges.attempts, 5),
          ),
        );
    },
    consume: async (id) =>
      Boolean(
        (
          await db
            ...
            .returning({ id: authLoginOtpChallenges.id })
        ).length,
      ),
    createSession: async (userId) => {
      sessionCookie.current = await createBetterAuthSession(userId, secret);
    },
  ...
  const response = NextResponse.json(
    result.ok
      ? { status: result.status, redirectTo: "/app/dashboard" }
      : result,
    { status: result.ok ? 200 : result.code === "RATE_LIMITED" ? 429 : 401 },
  );
  if (result.ok && sessionCookie.current) {
    response.cookies.set(
      sessionCookie.current.name,
      sessionCookie.current.value,
      sessionCookie.current.attributes,
    );
    response.cookies.set(TRUSTED_COOKIE, result.trustedCookie, {
      httpOnly: true,
