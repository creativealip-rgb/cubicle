...
export default async function AdminAnalyticsPage({ searchParams }: { searchParams: Promise<{ range?: string; view?: string }> }) {
  const params = await searchParams;
  const view = views.includes(params.view as View) ? params.view as View : "overview";
  const [data, spend] = await Promise.all([
    getAdminGrowthDashboard(params.range),
    view === "acquisition" ? listMarketingSpend() : Promise.resolve([]),
  ]);
