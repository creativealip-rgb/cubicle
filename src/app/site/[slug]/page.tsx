...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: site.title, description: site.hero };
}

function parseSite(body?: string | null): SiteData | null {
  try {
...
async function getSiteBySlug(slug: string) {
  ...
  for (const row of rows) {
    const site = parseSite(row.body);
    if (site?.published && site.slug === slug) return site;
  }
  return null;
}
