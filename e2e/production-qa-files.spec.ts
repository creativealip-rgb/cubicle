...
test("global file upload reload download delete", async ({ page }) => {
  ...
  await page.reload();
  await page.getByPlaceholder(/Cari folder dan file|Search folders and files/i).last().fill(fileName);
  const fileNameNode = page.getByText(fileName, { exact: true }).last();
  await expect(fileNameNode).toBeVisible();
  const actions = page.getByRole("button", { name: new RegExp(`${fileName} (actions|aksi)`, "i") });
  await actions.click();
  await page.evaluate(() => {
  ...
  await page.getByRole("menuitem", { name: /Unduh|Download/i }).click();
