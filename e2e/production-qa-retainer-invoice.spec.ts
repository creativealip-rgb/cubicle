...
test("production retainer invoice full amount then duplicate lock", async ({ page }) => {
  test.setTimeout(180_000);
  const stamp = Date.now();
  const client = `QA Retainer Invoice Client ${stamp}`;
  const project = `QA Retainer Invoice Project ${stamp}`;
  ...
  await page.getByRole("option", { name: "Retainer", exact: true }).click();
  const retainerFields = projectDialog.getByRole("spinbutton");
  await retainerFields.nth(0).fill("1000000");
  await retainerFields.nth(1).fill("120");
  await retainerFields.nth(2).fill("1");
  ...
  try {
    ...
    await source.click();
    await page.getByRole("option", { name: "Nilai penuh", exact: true }).click();
    ...
    await page.getByRole("button", { name: "Buat Invoice" }).click();
    await expect(page.getByText(/tidak ada sisa|no remaining|sisa/i).first()).toBeVisible({ timeout: 15000 });
    expect(page.url()).toMatch(/\/app\/invoices\/new$/);
  } finally {
    await page.goto("/app/projects");
