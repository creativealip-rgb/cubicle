...
export default function workload(){ const session=sessions[(__VU-1)%sessions.length]; const roll=Math.random(); const route=(cumulative.find(([,limit])=>roll<=limit)||cumulative[cumulative.length-1])[0]; const response=http.get(`${__ENV.BASE_URL}${paths[route]}`,{headers:{Cookie:session.cookie,Host:__ENV.TARGET_HOST||'app.cubiqlo.com'},tags:{route,slo:'light'}}); check(response,{expected_status:r=>profile.acceptedStatuses.includes(r.status)}); sleep(2+Math.random()*6); }
