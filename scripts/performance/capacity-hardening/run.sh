...
OUT="$ROOT/.capacity-runtime/$RUN_ID"; docker run --rm --user root --network host -v "$ROOT/scripts/performance/capacity-hardening:/capacity:ro" -v "$OUT:/runtime" "$IMAGE" run --out "json=/runtime/k6-raw.jsonl" -e PROFILE="$PROFILE" -e DURATION="${DURATION:-}" -e BASE_URL="$BASE_URL" -e SESSIONS_FILE=/runtime/sessions.json -e SUMMARY_FILE=/runtime/k6-summary.json /capacity/k6/workload.js
