...
if [[ ! "$TEST_DB" =~ ^cubicle_phase4_it(_[A-Za-z0-9_]+)?$ ]]; then
  echo "TEST_DB must use cubicle_phase4_it* disposable namespace" >&2
  exit 1
fi
...
source_phase4_tables=$(docker exec "$DB_CONTAINER" psql -U "$DB_USER" -d "$SOURCE_DB" -X -Atc \
  "SELECT count(*) FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('package_items','project_package_assignments')")
if [[ "$source_phase4_tables" != "0" && "$source_phase4_tables" != "2" ]]; then
  echo "Unexpected Phase 4 table count in $SOURCE_DB: $source_phase4_tables" >&2
  exit 1
fi
...
CREATE TABLE phase4_fixture_ids (prefix text NOT NULL, key text NOT NULL, value text NOT NULL);
WITH inserted_users AS (
  INSERT INTO users (id,name,email,email_verified,plan)
  VALUES
    (:'prefix'||'-user','Phase4 User',:'prefix'||'@example.test',true,'team'),
    (:'prefix'||'-other-user','Phase4 Other User',:'prefix'||'-other@example.test',true,'team')
  RETURNING id
), inserted_workspaces AS (
  INSERT INTO workspaces (name,slug,owner_id,default_currency)
  VALUES
    ('Phase4 Workspace',:'prefix',:'prefix'||'-user','IDR'),
    ('Phase4 Other Workspace',:'prefix'||'-other',:'prefix'||'-other-user','IDR')
  RETURNING id,slug
), inserted_members AS (
  ...
  INSERT INTO clients (workspace_id,name,portal_enabled,portal_slug,portal_slug_enabled)
  SELECT id,
    CASE WHEN slug=:'prefix' THEN :'prefix'||'-client' ELSE :'prefix'||'-other-client' END,
    true,
    CASE WHEN slug=:'prefix' THEN :'prefix'||'-portal-slug' ELSE :'prefix'||'-other-slug' END,
    true
  FROM inserted_workspaces
  RETURNING id,workspace_id,name,portal_slug
), inserted_other_project AS (
  ...
  SELECT c.workspace_id,c.id,:'prefix'||'-other-project','package','billable',true,'IDR',true
  ...
  INSERT INTO projects (workspace_id,client_id,name,billing_type,time_tracking_mode,activity_required,currency,client_visible)
  SELECT c.workspace_id,c.id,:'prefix'||'-project','package','billable',true,'IDR',true
  FROM inserted_clients c JOIN inserted_workspaces w ON w.id=c.workspace_id WHERE w.slug=:'prefix'
  UNION ALL
  SELECT c.workspace_id,c.id,:'prefix'||'-hidden-project','package','billable',true,'IDR',false
  FROM inserted_clients c JOIN inserted_workspaces w ON w.id=c.workspace_id WHERE w.slug=:'prefix'
  RETURNING id,workspace_id,client_id,name
)
INSERT INTO phase4_fixture_ids(prefix,key,value)
...
UNION ALL SELECT :'prefix','other_project_id',id::text FROM inserted_other_project WHERE name=:'prefix'||'-other-project'
UNION ALL SELECT :'prefix','hidden_project_id',id::text FROM inserted_projects WHERE name=:'prefix'||'-hidden-project'
UNION ALL SELECT :'prefix','client_id',client_id::text FROM inserted_projects WHERE name=:'prefix'||'-project'
UNION ALL SELECT :'prefix','portal_slug',portal_slug FROM inserted_clients WHERE name=:'prefix'||'-client'
UNION ALL SELECT :'prefix','user_id',:'prefix'||'-user';
SQL

docker exec -i "$DB_CONTAINER" psql -U "$DB_USER" -d "$TEST_DB" -X -v ON_ERROR_STOP=1 \
  < "$ROOT/drizzle/0050_package_builder.sql" >/dev/null
...
SELECT count(*) FROM phase4_fixture_ids;
RESET ROLE;
SQL
...
docker run --rm --network "$NETWORK" \
  ...
  "$NODE_IMAGE" \
  ./node_modules/.bin/tsx scripts/test-phase4-db-integration.mts

production_phase4_tables=$(docker exec "$DB_CONTAINER" psql -U "$DB_USER" -d cubicle -X -Atc \
  "SELECT count(*) FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('package_items','project_package_assignments')")
production_phase4_columns=$(docker exec "$DB_CONTAINER" psql -U "$DB_USER" -d cubicle -X -Atc \
  "SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND ((table_name='project_services' AND column_name IN ('package_item_id','project_package_assignment_id','source_package_assignment_id')) OR (table_name='package_orders' AND column_name='project_package_assignment_id'))")
...
[[ "$production_phase4_tables" == "0" ]]
[[ "$production_phase4_columns" == "0" ]]
