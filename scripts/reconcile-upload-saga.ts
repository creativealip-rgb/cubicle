...
async function main() {
  ...
  if (apply && objects.orphans.length) objects.deletedObjects = await deleteUploadOrphans(r2, { bucket: R2_BUCKET, keys: objects.orphans.slice(0, 100), olderThan: new Date(Date.now() - graceMs), limit: 100 });
