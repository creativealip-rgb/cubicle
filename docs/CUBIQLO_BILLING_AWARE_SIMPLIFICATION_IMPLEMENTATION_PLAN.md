...
No automatic `package → retainer` migration. `resolveBillingModel` rules: canonical known non-null value wins; null falls back from legacy `project→fixed_price`, `hours→hourly`, `package→legacy_package`; unknown or fully null fails closed. Every guard uses this resolver, never the nullable column directly.
...
export function resolveBillingModel(input: { billingModel: string | null | undefined; billingType: string | null | undefined }): BillingModel;
```
...
Server validation accepts only valid IANA timezone identifiers by constructing `Intl.DateTimeFormat` with the submitted value. Valid examples: `Asia/Jakarta`, `Asia/Makassar`, `Asia/Jayapura`, and `UTC`. Invalid arbitrary strings are rejected before persistence.

Cross-column validation is mandatory in server actions and DB constraint: Retainer requires fee >= 0, included minutes >= 0, period unit `month`, reset day 1–28, valid overage policy, and overage rate >= 0 when policy is `bill`. Hourly requires a resolvable Project/workspace rate before timer/manual entry creation. Retainer regular usage does not use hourly rate; only `bill` overage requires a period-snapshotted overage rate.

V1 decisions:

- Period unit: monthly only.
- Period boundary: calendar month anchored by `retainerResetDay` 1–28. `period_start` is inclusive and `period_end` is exclusive; reset day 10 means `2026-07-10 <= workDate < 2026-08-10`.
- Timezone: `workspaces.timezone`; schema default UTC. No user-timezone fallback in V1.
...
### Time-entry period linkage and consistency

Migration 0058 also adds an explicit tenant-safe relation:

```sql
ALTER TABLE time_entries ADD COLUMN IF NOT EXISTS retainer_period_id uuid;
ALTER TABLE time_entries ADD CONSTRAINT time_entries_retainer_period_workspace_fk
  FOREIGN KEY (retainer_period_id, workspace_id)
  REFERENCES retainer_periods(id, workspace_id) ON DELETE RESTRICT;
```

`retainer_periods` must expose `UNIQUE(id, workspace_id)` and use composite `FOREIGN KEY (project_id, workspace_id) REFERENCES projects(id, workspace_id) ON DELETE RESTRICT`. Classification table uses the same composite tenant FK. Server validates Project and work date fall inside period. Locked period rejects create/edit/reassign/delete/reversal.

Consistency strategy:

- Open period usage is calculated live from linked entries.
- No ad-hoc increment/decrement of aggregate minutes in time actions.
- Lock transaction uses `SELECT ... FOR UPDATE`, recalculates approved minutes from linked eligible entries, writes `approved_minutes` and `overage_minutes` once, then marks locked.
- Locked/invoiced period source entries are immutable.
- Configuration/timezone changes cannot move existing linked entries.
...
ALTER TABLE invoices
  ...
  ADD COLUMN IF NOT EXISTS retainer_period_id uuid;

ALTER TABLE invoices
  ADD CONSTRAINT invoices_retainer_period_workspace_fk
  FOREIGN KEY (retainer_period_id, workspace_id)
  REFERENCES retainer_periods(id, workspace_id)
  ON DELETE RESTRICT;
...
- `invoices.retainer_period_id` is the only authoritative invoice-period relation; `retainer_periods` has no writable `invoice_id`. Cancelling a draft/sent Retainer invoice sets period back to `locked` and increments `invoice_generation` on next generation.
...
## 11. Phase 0A — Additive Billing Compatibility, Then Server Containment

This phase must ship before or in the same release as any UI hiding. Ordering is mandatory: Phase 0A.1 applies additive migration 0056 and updates Drizzle schema; Phase 0A.2 deploys dual-read resolution and mutation guards. Retainer migrations 0057+ remain later.

### Phase 0A.1 — Migration compatibility

- Fetch all refs and inspect active worktrees.
- Reserve `0056` in `docs/migration-registry.md`.
- Create `drizzle/0056_billing_model_compatibility.sql`.
- Update `src/db/schema.ts` in the same commit.
- Apply to dev with `scripts/migrate-ledger.sh` and explicit `DB_NAME=cubicle_dev`.
- Verify migration ledger, column, check constraint, and backfill counts.
- Production remains untouched.
- Every manually declared constraint must be retry-safe using guarded `DO $$ ... IF NOT EXISTS ... $$` checks against `pg_constraint`; do not assume an operator will never retry the ledger command.

### Phase 0A.2 — Dual-read guards
...
```text
feat: add canonical billing model compatibility
fix: enforce billing-aware time mutation guards
```

### Rollback

Code rollback is safe because the old app ignores the additive nullable `billing_model` column. Leave migration 0056 and its populated column in place during the rollback window. Do not manually drop the column or rewind `docs/migration-registry.md`. If 0056 must later be retired before wider use, create a reviewed forward migration; never apply untracked rollback SQL.
...
SELECT p.id, p.name, te.status, (ii.id IS NOT NULL) AS invoiced,
       count(DISTINCT te.id) AS entries, sum(te.duration_minutes) AS minutes
FROM projects p
...
### Verified existing invoice source integrity
...
### Phase 3 — Retainer, Task, and Classification Schema
...
## 20. Baseline and Worktree Gate

Before implementation:

```bash
git fetch origin
git rev-parse origin/dev/integration
git status --short
